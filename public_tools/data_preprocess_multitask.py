...
def read_corpus(corpus_path, save_tags=False, save_attrs=False):
    """
    file format:
    中	B-ORG
    共	M-ORG
    ...
    for line in lines:
        if line != '\n':
            ...
            sent_.append(char)
            tag_.append(tag)
            attribute_.append(attr)
            # 统计tag词表和attr词表
            tags.append(tag)
            attrs.append(attr)
        else:
            data.append((sent_, tag_, attribute_))
            sent_, tag_, attribute_ = [], [], []
    ...
    for index, tag in enumerate(tags_set):
        tags_dic[tag] = index
    for index, attr in enumerate(attrs_set):
        attrs_dic[attr] = index

    if save_tags:
        # boson_ner/clue_ner/general_ner/msra_ner/renMinRiBao_ner/resume_ner
        with open('../data/clue_ner/multitask/bmeo2label.txt', 'w', encoding='utf-8') as f:
            for tag, index in tags_dic.items():
                f.write(tag + '\t' + str(index) + '\n')
    if save_attrs:
        # boson_ner/clue_ner/general_ner/msra_ner/renMinRiBao_ner/resume_ner
        with open('../data/clue_ner/multitask/attr2label.txt', 'w', encoding='utf-8') as f:
            for attr, index in attrs_dic.items():
                f.write(attr + '\t' + str(index) + '\n')
    return data
...
def load_attr2label(attr2label_path):
    attr2id = {}
    id2attr = {}
    with open(attr2label_path, 'r', encoding='utf-8') as f2:
        lines = f2.readlines()
        for line in lines:
            line = line.strip('\n').split('\t')
            attr = line[0]
            ...
            attr2id[attr] = index
            id2attr[index] = attr
    print('attr2id: {}'.format(attr2id))
    print('id2attr: {}'.format(id2attr))
    return attr2id, id2attr


def batch_yield(data, batch_size, vocab, tag2label, attr2label, shuffle=False):
    """
    ...
    seqs, tag_labels, attr_labels = [], [], []
    for (sent_, tag_, attribute_) in data:
        sent_ = sentence2id(sent_, vocab)

        tag_label_ = [tag2label[tag] for tag in tag_]
        attr_label_ = [attr2label[attr] for attr in attribute_]

        if len(seqs) == batch_size:
            yield seqs, tag_labels, attr_labels
            seqs, tag_labels, attr_labels = [], [], []

        seqs.append(sent_)
        tag_labels.append(tag_label_)
        attr_labels.append(attr_label_)
...
if __name__ == '__main__':
    ...
    read_corpus(clue_ner_corpus_path, save_tags=True, save_attrs=True)
