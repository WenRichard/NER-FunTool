...
class Metrics(object):
    ...
    def __init__(self, golden_tags, predict_tags, id2tag, remove_O=False, use_id2tag=True):

        # [[t1, t2], [t3, t4]...] --> [t1, t2, t3, t4...]
        if use_id2tag:
            self.golden_tags = self.flatten_lists(golden_tags)
            self.golden_tags = [id2tag[i] for i in self.golden_tags]
            self.predict_tags = self.flatten_lists(predict_tags)
            self.predict_tags = [id2tag[i] for i in self.predict_tags]
        else:
            self.golden_tags = self.flatten_lists(golden_tags)
            self.predict_tags = self.flatten_lists(predict_tags)
    ...
    def _remove_Stags(self):
        """
        s tag在数据集中出现的较少，容易出现数据不平衡的情况
        :return:
        """
        length = len(self.golden_tags)
        S_tag_indices = [i for i in range(length)
                         if self.golden_tags[i] == 'S']

        self.golden_tags = [tag for i, tag in enumerate(self.golden_tags)
                            if i not in S_tag_indices]

        self.predict_tags = [tag for i, tag in enumerate(self.predict_tags)
                             if i not in S_tag_indices]
        print("原总标记数为{}，移除了{}个S标记，占比{:.2f}%".format(
            length,
            len(S_tag_indices),
            len(S_tag_indices) / length * 100
        ))
